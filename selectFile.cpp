...
void selectFile::initSelector(string fdir){
  ...
  adr << std::fixed << fGui<<"/FreeSerif.ttf";
  ...
  selectAnnuler.setPosition(sf::Vector2f(360, 408));
  adr.clear();
  adr.str("");
  adr << std::fixed << fGui<<"/ok.png";
  selectBValider.loadFromFile(adr.str());
  selectValider.setTexture(selectBValider);
  selectValider.setPosition(sf::Vector2f(420, 408));
  adr.clear();
  adr.str("");
...
void selectFile::onClick(sf::Event e){
   ...
   if(e.mouseButton.x>0 && e.mouseButton.x<178 && e.mouseButton.y>36 && e.mouseButton.y<380){
     ...
     std::cout << " path :"<<adr.str()<<" listDirIndex "<<listDirsIndex2 <<" id= "<<id<< std::endl;
     if(id<vecDirs.size()){
	     selectType=0;
	     ...
	     listDirs(adr.str());
	     readFiles();
	     std::cout << "index dir: " <<vecDirs[id]<< " path :"<<adr.str()<<" listDirIndex "<<listDirsIndex2 <<" id= "<<id<< std::endl;
	     adr.clear();
	     adr.str("");
     }
   }
